...
def _read_state():
    """读取状态文件，返回 (mode, fresh)。文件缺失/不可读/损坏/非法都视为 ("off", False)。

    文件格式：第一行模式，可选第二行 "fresh" 标记。
    errors="replace" 防止损坏文件抛 UnicodeDecodeError 把 hook 搞崩。
    """
    try:
        ...
        return "off", False
    mode = lines[0].strip() if lines else "off"
    if mode not in ARM_MODES:
        return "off", False
    fresh = len(lines) > 1 and lines[1].strip() == "fresh"
    return mode, fresh
...
def is_fresh():
    """是否「刚布防」。布防命令本身是模型一个回合，回合结束会触发一次 Stop；
    fresh 标记让该次停下被吸收，布防从「下一次」停下才真正生效。"""
    return _read_state()[1]


def set_arm(mode):
    """设置布防模式并标记 fresh；off 时删除状态文件。无效模式抛 ValueError。"""
    if mode not in ARM_MODES:
    ...
    with open(STATE_FILE, "w", encoding="utf-8") as f:
        f.write(f"{mode}\nfresh")  # 布防生效从下次停下起，跳过布防回合自身的停下


def clear_fresh():
    """清除 fresh 标记（吸收掉布防回合自身的那次停下），保留模式。"""
    mode, fresh = _read_state()
